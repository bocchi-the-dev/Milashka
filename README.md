![milashka](https://github.com/bocchi-the-dev/banners/blob/main/milashka-banner.png?raw=true)

# Milashka

## What is Milashka?  
Milashka is a work-in-progress MEMEUI/HyperrrOS ROM that is focused on improving the user's experience over modifications under the hood.

## Usage Guidelines  
You are free to use this project for personal or commercial purposes, but please provide proper credit.  
Failure to do so may violate the **GNU General Public License v3 (GPL-3.0)**.

## Licensing
This project is licensed under the terms of the GNU General Public License v3.0. Some external dependencies are distributed under a different license, such as:

- [Uber Apk Signer](https://github.com/patrickfav/uber-apk-signer), licensed under **Apache 2.0**
- [avbtool](https://android.googlesource.com/platform/external/avb/+/master/avbtool.py), licensed under **Apache 2.0**
- [Apktool](https://github.com/iBotPeaches/Apktool), licensed under **Apache 2.0**
- [Magisk](https://github.com/topjohnwu/Magisk), licensed under **GPL 3.0**

---

## Takedowns
If you are not okay with me using your stuff or if you want your goodie(s) to be like
as you want in this repository, make an issue.